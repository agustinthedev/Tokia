...
When the implementation pull request is created, the workflow copies the issue's non-status labels, such as `source:codex`, `type:*`, and `priority:*`, to the pull request. Status labels remain on the issue because they describe the issue lifecycle rather than the pull request.
