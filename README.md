...
Preview generation is local and must complete before the final confirmation step. Generated media is stored under the configured content storage directory; originals are left untouched. Video slideshow exports use true 720p/1080p dimensions for the selected aspect ratio and variable-quality H.264/AAC encoding. The default `High quality` profile uses CRF 18 with a 12 Mbps peak rate; `Maximum quality` uses CRF 16 with a 20 Mbps peak rate. Scenes are composed and encoded in one FFmpeg pass, and smaller sources receive Lanczos upscaling plus selective sharpening. Pinterest image sources use the largest/original CDN variant when available, with resilient numeric fallbacks.
