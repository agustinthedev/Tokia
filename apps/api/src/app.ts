...
export async function buildApp(options: { db?: Database.Database; settings?: AppSettings } = {}): Promise<any> {
  ...
  function selectImagesForContent(projectId: string, contentId: string, requestedIds?: string[], shuffle = false): Row {
    ...
    const placeholders = sourceIds.map(() => '?').join(','); const imageRows = requestedIds ? db.prepare(`SELECT DISTINCT a.* FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE ca.collection_id IN (${placeholders}) AND a.id IN (${ids.map(() => '?').join(',')}) AND a.media_type IN ('image','animated') AND a.status = 'available' AND a.archived_at IS NULL`).all(...sourceIds, ...ids) as Row[] : db.prepare(`SELECT DISTINCT a.* FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','animated') AND a.status = 'available' AND a.archived_at IS NULL ORDER BY ${shuffle ? 'RANDOM()' : 'a.last_seen_at DESC'} LIMIT ?`).all(...sourceIds, required) as Row[];
    if (imageRows.length < required) throw new ContentValidationError('INSUFFICIENT_MEDIA', `The selected sources contain ${imageRows.length} usable images; ${required} are required.`);
  ...
  app.put('/api/content/:id/frames/:frameId/image', { schema: { tags: ['content'], summary: 'Replace one frame image' } }, async (request, reply) => {
    ...
    const candidate = db.prepare(`SELECT DISTINCT a.id FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE a.id = ? AND ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','animated') AND a.status = 'available' AND a.archived_at IS NULL`).get(mediaId, ...sourceIds.sourceCollectionIds);
