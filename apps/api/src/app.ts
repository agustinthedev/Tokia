...
import { contentSnapshot, createContentDraft, enqueueJob, startContentWorker, updateContent, validateSourceCollections } from './content-service.js';
import { contentDirectory } from './content-media.js';
import { createZip } from './zip.js';
import { CONTENT_TYPES, DEFAULT_CONFIGURATION, ContentValidationError, contentFrameCount, defaultFrameDuration, frameRoles, mergeConfiguration, normalizeFrameDuration, slugify, type ContentConfiguration } from './content-model.js';
...
function assetWhere(query: QueryRecord): { clauses: string[]; params: unknown[] } {
  ...
  if (query.mediaType === 'source') clauses.push("a.media_type IN ('image', 'video', 'animated')");
  else if (query.mediaType && ['image', 'video', 'animated'].includes(query.mediaType)) { clauses.push('a.media_type = ?'); params.push(query.mediaType); }
...
export async function buildApp(options: { db?: Database.Database; settings?: AppSettings } = {}): Promise<any> {
  ...
  await app.register(swagger, { openapi: { info: { title: 'Tokia Local Media API', version: '0.2.0', description: 'Local-first media library, imports, projects, and content generation API.' }, servers: [{ url: `http://${settings.host}:${settings.port}` }], tags: [{ name: 'diagnostics' }, { name: 'dashboard' }, { name: 'collections' }, { name: 'assets' }, { name: 'projects' }, { name: 'content' }, { name: 'imports' }, { name: 'search' }] } });
  ...
  app.get('/api/projects', { schema: { tags: ['projects'], summary: 'List projects' } }, async (request) => {
    ...
    const rows = db.prepare(`SELECT p.*, COUNT(DISTINCT pc.collection_id) AS collection_count, COUNT(DISTINCT ca.asset_id) AS total_assets, COUNT(DISTINCT CASE WHEN a.media_type IN ('image','animated') THEN a.id END) AS image_count, COUNT(DISTINCT CASE WHEN a.media_type = 'video' THEN a.id END) AS video_count,
      (SELECT COUNT(*) FROM content_items ci WHERE ci.project_id = p.id AND ci.status != 'archived') AS content_count,
      (SELECT COUNT(*) FROM content_items ci WHERE ci.project_id = p.id AND ci.status = 'draft') AS draft_count,
      (SELECT COUNT(*) FROM content_items ci WHERE ci.project_id = p.id AND ci.status IN ('preview_generating','generation_queued','generating')) AS generating_count,
      (SELECT COUNT(*) FROM content_items ci WHERE ci.project_id = p.id AND ci.status = 'ready') AS ready_count
      FROM projects p LEFT JOIN project_collections pc ON pc.project_id = p.id LEFT JOIN collection_assets ca ON ca.collection_id = pc.collection_id LEFT JOIN assets a ON a.id = ca.asset_id AND a.archived_at IS NULL WHERE ${where} GROUP BY p.id ORDER BY p.updated_at DESC LIMIT ? OFFSET ?`).all(...params, pageSize, (page - 1) * pageSize) as Row[];
  ...
  app.post('/api/projects', { schema: { tags: ['projects'], summary: 'Create a project' } }, async (request, reply) => {
    ...
    const configJson = body.defaultSettings && typeof body.defaultSettings === 'object' ? JSON.stringify(body.defaultSettings) : body.config && typeof body.config === 'object' ? JSON.stringify(body.config) : JSON.stringify(DEFAULT_CONFIGURATION);
    const slug = `${slugify(name)}-${id.slice(0, 8)}`;
    db.transaction(() => {
      db.prepare('INSERT INTO projects(id, name, description, niche, default_language, internal_notes, color, slug, status, cover_asset_id, config_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(id, name, text(body.description), niche, text(body.defaultLanguage, 80) ?? 'English', text(body.internalNotes), text(body.color, 30), slug, status, text(body.coverAssetId, 100), configJson, timestamp, timestamp);
  ...
  app.patch('/api/projects/:id/archive', { schema: { tags: ['projects'], summary: 'Archive a project' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return;
    const { id } = request.params as { id: string }; const existing = db.prepare('SELECT id FROM projects WHERE id = ?').get(id);
    if (!existing) return reply.code(404).send({ error: { code: 'PROJECT_NOT_FOUND', message: 'Project not found' } });
    db.prepare("UPDATE projects SET status = 'archived', archived_at = COALESCE(archived_at, ?), updated_at = ? WHERE id = ?").run(now(), now(), id);
    return projectSnapshot(db, id);
  });

  app.delete('/api/projects/:id', { schema: { tags: ['projects'], summary: 'Archive a project without deleting its sources' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return;
    const { id } = request.params as { id: string }; const existing = db.prepare('SELECT id FROM projects WHERE id = ?').get(id);
    if (!existing) return reply.code(404).send({ error: { code: 'PROJECT_NOT_FOUND', message: 'Project not found' } });
    db.prepare("UPDATE projects SET status = 'archived', archived_at = COALESCE(archived_at, ?), updated_at = ? WHERE id = ?").run(now(), now(), id);
  ...
  function assertContentEditable(content: Row): void {
    if (content.status === 'generation_queued' || content.status === 'generating') throw new ContentValidationError('CONTENT_BUSY', 'Final generation is running; wait for it to finish before editing.');
  }
  function contentResult(contentId: string, job?: Row): Row { return { content: contentSnapshot(db, contentId), job: job ? { id: job.id, jobType: job.job_type, status: job.status, progress: job.progress, attempt: job.attempt } : null }; }

  app.get('/api/projects/:id/content', { schema: { tags: ['content'], summary: 'List content items in a project' } }, async (request, reply) => {
    const { id } = request.params as { id: string }; projectExists(id); const query = queryOf(request); const page = positiveInt(query.page, 1, 100_000); const pageSize = positiveInt(query.pageSize, 24, 100);
    const clauses = ['c.project_id = ?', 'c.status != \'archived\'']; const params: unknown[] = [id];
    if (query.status) { clauses.push('c.status = ?'); params.push(query.status); }
    if (query.search?.trim()) { clauses.push('(LOWER(COALESCE(c.title, \'\')) LIKE LOWER(?) OR LOWER(COALESCE(c.topic, \'\')) LIKE LOWER(?))'); const value = `%${query.search.trim()}%`; params.push(value, value); }
    const where = clauses.join(' AND '); const total = (db.prepare(`SELECT COUNT(*) AS count FROM content_items c WHERE ${where}`).get(...params) as { count: number }).count;
    const rows = db.prepare(`SELECT c.*, COUNT(DISTINCT f.id) AS frame_count, (SELECT ca.id FROM content_assets ca WHERE ca.content_id = c.id AND ca.variant IN ('final','preview') AND ca.asset_type IN ('thumbnail','video','image') ORDER BY CASE ca.variant WHEN 'final' THEN 0 ELSE 1 END, CASE ca.asset_type WHEN 'thumbnail' THEN 0 ELSE 1 END, ca.created_at DESC LIMIT 1) AS thumbnail_asset_id FROM content_items c LEFT JOIN content_frames f ON f.content_id = c.id WHERE ${where} GROUP BY c.id ORDER BY c.updated_at DESC LIMIT ? OFFSET ?`).all(...params, pageSize, (page - 1) * pageSize) as Row[];
    return { items: rows.map((row) => ({ id: row.id, projectId: row.project_id, type: row.type, title: row.title, status: row.status, language: row.language, topic: row.topic, frameCount: row.frame_count, previewVersion: row.preview_version, errorCode: row.error_code, errorMessage: row.error_message, createdAt: row.created_at, updatedAt: row.updated_at, thumbnailUrl: row.thumbnail_asset_id ? `/api/content/${row.id}/assets/${row.thumbnail_asset_id}/preview` : null })), pagination: pagination(page, pageSize, total) };
  });

  app.post('/api/projects/:id/content', { schema: { tags: ['content'], summary: 'Create a content draft' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return;
    const { id } = request.params as { id: string }; projectExists(id); const body = bodyOf(request);
    const draft = createContentDraft(db, { projectId: id, type: body.type, title: body.title, language: body.language, configuration: body.configuration ?? body.config });
    if (body.autoSelect === true) selectImagesForContent(id, String((draft as Row).id), undefined, false);
    return reply.code(201).send(contentSnapshot(db, String((draft as Row).id)));
  });
  ...
  app.patch('/api/content/:id', { schema: { tags: ['content'], summary: 'Update content draft' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return;
    const { id } = request.params as { id: string }; const updated = updateContent(db, id, bodyOf(request)); return updated;
  });
  ...
  function queueNarrative(contentId: string, jobType: 'narrative_generation' | 'caption_regeneration' | 'frame_regeneration', input: Row = {}): Row {
    const content = contentExists(contentId); return enqueueJob(db, contentId, jobType, { promptVersion: 'structured-narrative-v1', configuration: content.configuration, ...input });
  }
  app.post('/api/content/:id/narrative', { schema: { tags: ['content'], summary: 'Generate structured narrative copy' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; return reply.code(202).send(contentResult(id, queueNarrative(id, 'narrative_generation'))); });
  app.post('/api/content/:id/narrative/regenerate', { schema: { tags: ['content'], summary: 'Regenerate all unlocked narrative copy' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; return reply.code(202).send(contentResult(id, queueNarrative(id, 'narrative_generation', { regenerate: true }))); });
  app.post('/api/content/:id/caption', { schema: { tags: ['content'], summary: 'Regenerate caption' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; return reply.code(202).send(contentResult(id, queueNarrative(id, 'caption_regeneration'))); });
  app.post('/api/content/:id/frames/:frameId/regenerate', { schema: { tags: ['content'], summary: 'Regenerate one frame' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id, frameId } = request.params as { id: string; frameId: string }; if (!db.prepare('SELECT 1 FROM content_frames WHERE id = ? AND content_id = ?').get(frameId, id)) return reply.code(404).send({ error: { code: 'FRAME_NOT_FOUND', message: 'Frame not found' } }); return reply.code(202).send(contentResult(id, queueNarrative(id, 'frame_regeneration', { frameId }))); });

  app.patch('/api/content/:id/frames/:frameId', { schema: { tags: ['content'], summary: 'Edit or lock one content frame' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return; const { id, frameId } = request.params as { id: string; frameId: string }; const content = contentExists(id); assertContentEditable(content); const body = bodyOf(request); const existing = db.prepare('SELECT * FROM content_frames WHERE id = ? AND content_id = ?').get(frameId, id) as Row | undefined; if (!existing) return reply.code(404).send({ error: { code: 'FRAME_NOT_FOUND', message: 'Frame not found' } });
    const headline = body.headline === undefined ? existing.headline : typeof body.headline === 'string' ? body.headline.trim().slice(0, 120) || null : null; const bodyText = body.body === undefined ? existing.body : typeof body.body === 'string' ? body.body.trim().slice(0, 240) || null : null; const textLocked = body.textLocked === undefined ? existing.text_locked : body.textLocked ? 1 : 0; const imageLocked = body.imageLocked === undefined ? existing.image_locked : body.imageLocked ? 1 : 0;
    let frameSettings = parseJson<Row>(existing.settings_json, {});
    if (body.durationSeconds !== undefined) {
      if (content.type !== 'video_slideshow') throw new ContentValidationError('DURATION_NOT_SUPPORTED', 'Per-frame duration is only available for video slideshows.');
      const source = existing.source_media_id ? db.prepare('SELECT media_type, duration_seconds FROM assets WHERE id = ?').get(existing.source_media_id) as Row | undefined : undefined;
      frameSettings = { ...frameSettings, durationSeconds: normalizeFrameDuration(body.durationSeconds, content.configuration as ContentConfiguration, source?.media_type, source?.duration_seconds), durationCustomized: true };
    }
    db.prepare('UPDATE content_frames SET headline = ?, body = ?, text_locked = ?, image_locked = ?, settings_json = ?, updated_at = ? WHERE id = ?').run(headline, bodyText, textLocked, imageLocked, JSON.stringify(frameSettings), now(), frameId); db.prepare('UPDATE content_items SET version = version + 1, updated_at = ? WHERE id = ?').run(now(), id); return contentSnapshot(db, id);
  });

  function selectImagesForContent(projectId: string, contentId: string, requestedIds?: string[], shuffle = false): Row {
    const content = contentExists(contentId); if (content.projectId !== projectId) throw new ContentValidationError('CONTENT_PROJECT_MISMATCH', 'Content item does not belong to this project.');
    const sourceIds = (content.configuration as { sourceCollectionIds: string[] }).sourceCollectionIds; validateSourceCollections(db, projectId, sourceIds); const frames = (db.prepare('SELECT * FROM content_frames WHERE content_id = ? ORDER BY position').all(contentId) as Row[]); const required = frames.length;
    const ids = requestedIds ?? [];
    if (requestedIds && (ids.length !== required || new Set(ids).size !== ids.length)) throw new ContentValidationError('INVALID_IMAGE_SELECTION', `Select exactly ${required} different images.`);
    const placeholders = sourceIds.map(() => '?').join(','); const imageRows = requestedIds ? db.prepare(`SELECT DISTINCT a.* FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE ca.collection_id IN (${placeholders}) AND a.id IN (${ids.map(() => '?').join(',')}) AND a.media_type IN ('image','video','animated') AND a.status = 'available' AND a.archived_at IS NULL`).all(...sourceIds, ...ids) as Row[] : db.prepare(`SELECT DISTINCT a.* FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','video','animated') AND a.status = 'available' AND a.archived_at IS NULL ORDER BY ${shuffle ? 'RANDOM()' : 'a.last_seen_at DESC'} LIMIT ?`).all(...sourceIds, required) as Row[];
    if (imageRows.length < required) throw new ContentValidationError('INSUFFICIENT_MEDIA', `The selected sources contain ${imageRows.length} usable media items; ${required} are required.`);
    const selected = requestedIds ? ids.map((requestedId) => imageRows.find((image) => image.id === requestedId)!).filter(Boolean) : imageRows.slice(0, required);
    db.transaction(() => { const timestamp = now(); const configuration = content.configuration as ContentConfiguration; for (let index = 0; index < frames.length; index += 1) { const frame = frames[index]!; const asset = selected[index]!; const settings = { ...parseJson<Row>(frame.settings_json, {}), durationSeconds: defaultFrameDuration(configuration, asset.media_type, asset.duration_seconds), durationCustomized: false }; db.prepare('UPDATE content_frames SET source_media_id = ?, settings_json = ?, updated_at = ? WHERE id = ?').run(asset.id, JSON.stringify(settings), timestamp, frame.id); db.prepare("DELETE FROM content_assets WHERE frame_id = ? AND variant = 'source_normalized'").run(frame.id); } db.prepare("UPDATE content_items SET status = 'draft', version = version + 1, updated_at = ?, error_code = NULL, error_message = NULL WHERE id = ?").run(timestamp, contentId); })();
    return contentSnapshot(db, contentId)!;
  }
  app.post('/api/content/:id/images/select', { schema: { tags: ['content'], summary: 'Select images for content frames' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; const content = contentExists(id); return selectImagesForContent(content.projectId as string, id, Array.isArray(bodyOf(request).mediaIds) ? (bodyOf(request).mediaIds as unknown[]).filter((value): value is string => typeof value === 'string') : undefined, false); });
  app.post('/api/content/:id/images/shuffle', { schema: { tags: ['content'], summary: 'Shuffle unlocked images' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; const content = contentExists(id); const frames = db.prepare('SELECT * FROM content_frames WHERE content_id = ? ORDER BY position').all(id) as Row[]; const locked = frames.filter((frame) => frame.image_locked); const unlocked = frames.filter((frame) => !frame.image_locked); const lockedIds = new Set(locked.map((frame) => frame.source_media_id).filter(Boolean)); const sourceIds = (content.configuration as { sourceCollectionIds: string[] }).sourceCollectionIds; const placeholders = sourceIds.map(() => '?').join(','); const candidates = db.prepare(`SELECT DISTINCT a.* FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','video','animated') AND a.status = 'available' AND a.archived_at IS NULL ORDER BY RANDOM() LIMIT ?`).all(...sourceIds, frames.length * 4) as Row[]; const available = candidates.filter((asset) => !lockedIds.has(asset.id)); if (available.length < unlocked.length) throw new ContentValidationError('INSUFFICIENT_MEDIA', 'There are not enough unlocked images to shuffle these frames.'); const selected = available.slice(0, unlocked.length); db.transaction(() => { const configuration = content.configuration as ContentConfiguration; for (let index = 0; index < unlocked.length; index += 1) { const frame = unlocked[index]!; const asset = selected[index]!; const frameSettings = { ...parseJson<Row>(frame.settings_json, {}), durationSeconds: defaultFrameDuration(configuration, asset.media_type, asset.duration_seconds), durationCustomized: false }; db.prepare('UPDATE content_frames SET source_media_id = ?, settings_json = ?, updated_at = ? WHERE id = ?').run(asset.id, JSON.stringify(frameSettings), now(), frame.id); db.prepare("DELETE FROM content_assets WHERE frame_id = ? AND variant = 'source_normalized'").run(frame.id); } db.prepare('UPDATE content_items SET status = \'draft\', version = version + 1, updated_at = ? WHERE id = ?').run(now(), id); })(); return contentSnapshot(db, id);
  });
  app.put('/api/content/:id/frames/:frameId/image', { schema: { tags: ['content'], summary: 'Replace one frame image' } }, async (request, reply) => {
    if (!integrationGuard(settings, request, reply)) return;
    const { id, frameId } = request.params as { id: string; frameId: string }; const content = contentExists(id); const mediaId = bodyOf(request).mediaId;
    if (typeof mediaId !== 'string') throw new ContentValidationError('INVALID_IMAGE_SELECTION', 'mediaId is required.');
    const frame = db.prepare('SELECT position FROM content_frames WHERE id = ? AND content_id = ?').get(frameId, id) as Row | undefined; if (!frame) throw new ContentValidationError('FRAME_NOT_FOUND', 'Frame not found.');
    const sourceIds = content.configuration as { sourceCollectionIds: string[] }; const placeholders = sourceIds.sourceCollectionIds.map(() => '?').join(',');
    const candidate = db.prepare(`SELECT DISTINCT a.id, a.media_type, a.duration_seconds FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE a.id = ? AND ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','video','animated') AND a.status = 'available' AND a.archived_at IS NULL`).get(mediaId, ...sourceIds.sourceCollectionIds) as Row | undefined;
    if (!candidate) throw new ContentValidationError('INVALID_IMAGE_SELECTION', 'The replacement image is not available in the project sources.');
    if (db.prepare('SELECT 1 FROM content_frames WHERE content_id = ? AND source_media_id = ? AND id != ?').get(id, mediaId, frameId)) throw new ContentValidationError('DUPLICATE_IMAGE_SELECTION', 'An image cannot be used twice in one content item.');
    db.transaction(() => { const settings = { ...parseJson<Row>(frame.settings_json, {}), durationSeconds: defaultFrameDuration(content.configuration as ContentConfiguration, candidate?.media_type, candidate?.duration_seconds), durationCustomized: false }; db.prepare('UPDATE content_frames SET source_media_id = ?, settings_json = ?, updated_at = ? WHERE id = ?').run(mediaId, JSON.stringify(settings), now(), frameId); db.prepare("DELETE FROM content_assets WHERE frame_id = ? AND variant = 'source_normalized'").run(frameId); db.prepare("UPDATE content_items SET status = 'draft', version = version + 1, updated_at = ? WHERE id = ?").run(now(), id); })(); return contentSnapshot(db, id);
  });
  app.post('/api/content/:id/frames/reorder', { schema: { tags: ['content'], summary: 'Reorder content frames' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; contentExists(id); const frameIds = Array.isArray(bodyOf(request).frameIds) ? (bodyOf(request).frameIds as unknown[]).filter((value): value is string => typeof value === 'string') : []; const existing = (db.prepare('SELECT id FROM content_frames WHERE content_id = ? ORDER BY position').all(id) as Row[]).map((row) => row.id as string); if (frameIds.length !== existing.length || new Set(frameIds).size !== existing.length || frameIds.some((value) => !existing.includes(value))) throw new ContentValidationError('INVALID_FRAME_ORDER', 'Frame order must include each frame exactly once.'); db.transaction(() => { for (let index = 0; index < frameIds.length; index += 1) db.prepare('UPDATE content_frames SET position = ?, updated_at = ? WHERE id = ?').run(-(index + 1), now(), frameIds[index]); for (let index = 0; index < frameIds.length; index += 1) db.prepare('UPDATE content_frames SET position = ?, updated_at = ? WHERE id = ?').run(index + 1, now(), frameIds[index]); db.prepare('UPDATE content_items SET version = version + 1, updated_at = ? WHERE id = ?').run(now(), id); })(); return contentSnapshot(db, id); });

  app.post('/api/content/:id/preview', { schema: { tags: ['content'], summary: 'Queue a preview render' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; const content = contentExists(id); const missing = (content.frames as Row[]).filter((frame) => !frame.sourceMedia); if (missing.length) throw new ContentValidationError('MISSING_SOURCE_IMAGE', 'Select an image for every frame before generating a preview.'); return reply.code(202).send(contentResult(id, enqueueJob(db, id, 'preview_render', { configurationVersion: content.version }))); });
  app.get('/api/content/:id/preview', { schema: { tags: ['content'], summary: 'Get preview state' } }, async (request, reply) => { const { id } = request.params as { id: string }; const content = contentSnapshot(db, id); if (!content) return reply.code(404).send({ error: { code: 'CONTENT_NOT_FOUND', message: 'Content item not found' } }); return content; });
  app.post('/api/content/:id/confirm', { schema: { tags: ['content'], summary: 'Confirm preview and queue final generation' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; const content = contentExists(id); if (content.status !== 'preview_ready' || !content.previewVersion) throw new ContentValidationError('PREVIEW_REQUIRED', 'Generate and review a preview before final generation.'); db.prepare('UPDATE content_items SET accepted_version = preview_version, updated_at = ? WHERE id = ?').run(now(), id); const job = enqueueJob(db, id, 'final_render', { acceptedVersion: content.previewVersion }); return reply.code(202).send(contentResult(id, job)); });
  app.post('/api/content/:id/retry', { schema: { tags: ['content'], summary: 'Retry a failed content job' } }, async (request, reply) => { if (!integrationGuard(settings, request, reply)) return; const { id } = request.params as { id: string }; const content = contentExists(id); const jobType = bodyOf(request).jobType === 'preview_render' || bodyOf(request).jobType === 'final_render' ? bodyOf(request).jobType as 'preview_render' | 'final_render' : content.previewVersion ? 'final_render' : 'preview_render'; db.prepare("UPDATE content_items SET status = 'draft', error_code = NULL, error_message = NULL, updated_at = ? WHERE id = ?").run(now(), id); const job = enqueueJob(db, id, jobType, { retry: true }); return reply.code(202).send(contentResult(id, job)); });

  function contentAsset(contentId: string, assetId: string): Row {
    const asset = db.prepare('SELECT * FROM content_assets WHERE id = ? AND content_id = ? AND status = \'ready\'').get(assetId, contentId) as Row | undefined; if (!asset || !fs.existsSync(String(asset.file_path))) throw new ContentValidationError('CONTENT_ASSET_NOT_FOUND', 'Generated asset is not available.'); return asset;
  }
  app.get('/api/content/:id/assets/:assetId/preview', async (request, reply) => { const { id, assetId } = request.params as { id: string; assetId: string }; const asset = contentAsset(id, assetId); return reply.type(String(asset.mime_type)).send(fs.createReadStream(String(asset.file_path))); });
  app.get('/api/content/:id/assets/:assetId/download', async (request, reply) => { const { id, assetId } = request.params as { id: string; assetId: string }; const asset = contentAsset(id, assetId); const ext = asset.mime_type === 'video/mp4' ? 'mp4' : asset.mime_type === 'image/webp' ? 'webp' : 'png'; return reply.header('Content-Disposition', `attachment; filename="tokia-${id}-${asset.variant}.${ext}"`).type(String(asset.mime_type)).send(fs.createReadStream(String(asset.file_path))); });
  app.get('/api/content/:id/download', async (request, reply) => {
    const { id } = request.params as { id: string };
    const content = contentExists(id);
    if (content.type === 'carousel') {
      const rows = db.prepare(`SELECT ca.* FROM content_assets ca LEFT JOIN content_frames f ON f.id = ca.frame_id WHERE ca.content_id = ? AND ca.variant = 'final' AND ca.asset_type = 'image' AND ca.status = 'ready' ORDER BY f.position, ca.created_at`).all(id) as Row[];
      const files = rows.filter((row) => fs.existsSync(String(row.file_path))).map((row, index) => ({ name: `slide-${String(index + 1).padStart(2, '0')}.png`, path: String(row.file_path) }));
      if (!files.length) throw new ContentValidationError('FINAL_ASSET_NOT_READY', 'The final carousel slides are not ready yet.');
      const archive = await createZip(files);
      return reply.header('Content-Disposition', `attachment; filename="${safeFileName(String(content.title ?? content.topic ?? 'content'))}-slides.zip"`).type('application/zip').send(archive);
    }
    const preferred = content.type === 'video_slideshow' ? 'video' : 'image';
    const asset = db.prepare(`SELECT * FROM content_assets WHERE content_id = ? AND variant = 'final' AND asset_type = ? AND status = 'ready' ORDER BY created_at DESC LIMIT 1`).get(id, preferred) as Row | undefined;
    if (!asset) throw new ContentValidationError('FINAL_ASSET_NOT_READY', 'The final asset is not ready yet.');
    const ext = asset.mime_type === 'video/mp4' ? 'mp4' : asset.mime_type === 'image/webp' ? 'webp' : 'png';
    return reply.header('Content-Disposition', `attachment; filename="${safeFileName(String(content.title ?? content.topic ?? 'content'))}.${ext}"`).type(String(asset.mime_type)).send(fs.createReadStream(String(asset.file_path)));
  });
  app.get('/api/content/:id/package.zip', async (request, reply) => { const { id } = request.params as { id: string }; const content = contentExists(id); if (content.status !== 'ready') throw new ContentValidationError('FINAL_ASSET_NOT_READY', 'The final asset is not ready yet.'); const rows = db.prepare("SELECT * FROM content_assets WHERE content_id = ? AND variant = 'final' AND status = 'ready' ORDER BY CASE asset_type WHEN 'image' THEN 0 ELSE 1 END, created_at").all(id) as Row[]; const files = rows.filter((row) => fs.existsSync(String(row.file_path))).map((row, index) => ({ name: row.asset_type === 'image' ? `slide-${String(index + 1).padStart(2, '0')}.png` : row.asset_type === 'video' ? 'video.mp4' : 'thumbnail.webp', path: String(row.file_path) })); const directory = contentDirectory(settings.contentStorageDirectory, id); const metadataPath = path.join(directory, 'metadata.json'); const captionPath = path.join(directory, 'caption.txt'); fs.writeFileSync(metadataPath, JSON.stringify({ id: content.id, projectId: content.projectId, type: content.type, title: content.title, topic: content.topic, language: content.language, configuration: content.configuration, narrative: content.narrative }, null, 2)); fs.writeFileSync(captionPath, `${content.narrative?.caption ?? ''}\n\n${(content.narrative?.hashtags ?? []).join(' ')}`); files.push({ name: 'metadata.json', path: metadataPath }, { name: 'caption.txt', path: captionPath }); const archive = await createZip(files); return reply.header('Content-Disposition', `attachment; filename="${safeFileName(String(content.title ?? 'content'))}.zip"`).type('application/zip').send(archive); });

  function safeFileName(value: string): string { return value.normalize('NFKD').replace(/[^a-zA-Z0-9]+/g, '-').replace(/^-+|-+$/g, '').toLowerCase().slice(0, 80) || 'content'; }
  ...
  app.addHook('onClose', async () => { stopContentWorker(); if (ownsDatabase) db.close(); });
  app.setErrorHandler((error, request, reply) => {
    ...
    if (error instanceof ContentValidationError) { request.log.warn({ requestId: request.id, code: error.code }, error.message); return reply.code(error.code === 'PROJECT_NOT_FOUND' || error.code === 'CONTENT_NOT_FOUND' ? 404 : error.code === 'CONTENT_BUSY' ? 409 : 400).send({ error: { code: error.code, message: error.message, requestId: request.id } }); }
