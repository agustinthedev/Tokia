...
export async function buildApp(options: { db?: Database.Database; settings?: AppSettings } = {}): Promise<any> {
  ...
  app.patch('/api/content/:id/frames/:frameId', { schema: { tags: ['content'], summary: 'Edit or lock one content frame' } }, async (request, reply) => {
    ...
    db.prepare('UPDATE content_frames SET headline = ?, body = ?, text_locked = ?, image_locked = ?, updated_at = ? WHERE id = ?').run(headline, bodyText, textLocked, imageLocked, now(), frameId); db.prepare('UPDATE content_items SET version = version + 1, updated_at = ? WHERE id = ?').run(now(), id); return contentSnapshot(db, id);
  });
  ...
  function selectImagesForContent(projectId: string, contentId: string, requestedIds?: string[], shuffle = false): Row {
    ...
    db.transaction(() => { const timestamp = now(); for (let index = 0; index < frames.length; index += 1) { const frame = frames[index]!; const asset = selected[index]!; db.prepare('UPDATE content_frames SET source_media_id = ?, updated_at = ? WHERE id = ?').run(asset.id, timestamp, frame.id); db.prepare("DELETE FROM content_assets WHERE frame_id = ? AND variant = 'source_normalized'").run(frame.id); } db.prepare("UPDATE content_items SET status = 'draft', version = version + 1, updated_at = ?, error_code = NULL, error_message = NULL WHERE id = ?").run(timestamp, contentId); })();
  ...
  app.put('/api/content/:id/frames/:frameId/image', { schema: { tags: ['content'], summary: 'Replace one frame image' } }, async (request, reply) => {
    ...
    const candidate = db.prepare(`SELECT DISTINCT a.id FROM assets a JOIN collection_assets ca ON ca.asset_id = a.id WHERE a.id = ? AND ca.collection_id IN (${placeholders}) AND a.media_type IN ('image','video','animated') AND a.status = 'available' AND a.archived_at IS NULL`).get(mediaId, ...sourceIds.sourceCollectionIds);
    ...
    db.transaction(() => { db.prepare('UPDATE content_frames SET source_media_id = ?, updated_at = ? WHERE id = ?').run(mediaId, now(), frameId); db.prepare("DELETE FROM content_assets WHERE frame_id = ? AND variant = 'source_normalized'").run(frameId); db.prepare("UPDATE content_items SET status = 'draft', version = version + 1, updated_at = ? WHERE id = ?").run(now(), id); })(); return contentSnapshot(db, id);
  });
