...
export async function buildApp(options: { db?: Database.Database; settings?: AppSettings } = {}): Promise<any> {
  ...
  app.get('/api/content/:id/download', async (request, reply) => { const { id } = request.params as { id: string; }; const content = contentExists(id); const preferred = content.type === 'video_slideshow' ? 'video' : content.type === 'single_image' ? 'image' : 'thumbnail'; const asset = db.prepare(`SELECT * FROM content_assets WHERE content_id = ? AND variant = 'final' AND asset_type = ? AND status = 'ready' ORDER BY created_at DESC LIMIT 1`).get(id, preferred) as Row | undefined; if (!asset) throw new ContentValidationError('FINAL_ASSET_NOT_READY', 'The final asset is not ready yet.'); const ext = asset.mime_type === 'video/mp4' ? 'mp4' : asset.mime_type === 'image/webp' ? 'webp' : 'png'; return reply.header('Content-Disposition', `attachment; filename="${safeFileName(String(content.title ?? content.topic ?? 'content'))}.${ext}"`).type(String(asset.mime_type)).send(fs.createReadStream(String(asset.file_path))); });
